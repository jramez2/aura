-- AuraApp: Migration to allow user-specific categories in Budget

-- 1. Add UsuarioId to CategoriasGasto
ALTER TABLE `CategoriasGasto` 
ADD `UsuarioId` BIGINT(20) NULL DEFAULT NULL AFTER `CategoriaPadreId`;

-- 2. Update Database View to include IDs and fix data retrieval
...
-- 3. Update Stored Procedure to include user categories
DELIMITER $$
DROP PROCEDURE IF EXISTS `Sp_InicializarPresupuestoUsuario`$$
CREATE PROCEDURE `Sp_InicializarPresupuestoUsuario` (IN `p_UsuarioId` BIGINT, IN `p_Anio` INT, IN `p_Mes` INT)   BEGIN
